...
class RuleGeneratorDialog(QDialog):
    ...
    def _get_merge_areas(self, sheet):
        ...
        try:
            ...
            rows = min(used_range.Rows.Count, 200)
            cols = min(used_range.Columns.Count, 30)
            processed = set()
            for r in range(1, rows + 1):
                for c in range(1, cols + 1):
                    try:
                        cell = sheet.Cells(r, c)
                        if hasattr(cell, 'MergeCells') and cell.MergeCells:
                            if hasattr(cell, 'MergeArea'):
                                ...
                                if key not in processed:
                                    ...
                                    area_cols = area.Columns.Count
                                    if area_rows == 1 and area_cols == 1:
                                        continue
                                    merge_areas.append({
                                        'Row': area.Row,
                                        'Column': area.Column,
    ...
    def _load_sheet_data(self, sheet):
        ...
        self.logger.info(f"检测到 {len(merge_areas) if merge_areas else 0} 个有效合并区域")

        max_row, max_col = self._scan_data_range(sheet, merge_areas)
        self.logger.info(f"实际数据范围: {max_row} 行, {max_col} 列")

        self._populate_table(sheet, max_row, max_col, merge_areas)
    ...
    def _scan_data_range(self, sheet, merge_areas):
        ...
        if merge_areas:
            for area in merge_areas:
                rows = area['Rows']
                cols = area['Columns']
                for r in range(area['Row'], area['Row'] + rows):
                    merge_rows.add(r)
                for c in range(area['Column'], area['Column'] + cols):
                    merge_cols.add(c)
        ...
        if max_row == 0:
            ...
            for r in range(1, max_row + 1):
                for c in range(1, max_col + 5):
                    ...
                    cell = sheet.Cells(r, c)
                    val = cell.Text
                    if val and str(val).strip() and c > max_col:
                        max_col = c
            if merge_cols:
    ...
    # ============================================================
    # 填充表格（保留合并，过滤单格，重置合并状态）
    # ============================================================
    def _populate_table(self, sheet, max_row, max_col, merge_areas):
        self.logger.info(f"_populate_table: 开始填充 {max_row}x{max_col}")
        ...
        for r in range(max_row):
            ...
            for c in range(max_col):
                ...
                if display and "（" in display and "-" in display and "）" in display:
        ...
        self.logger.info("_populate_table: 完成")
